...
- **Engine layer (`src/engine/`)** — `createStudio()` boots the real openDAW engine (Workers → AudioWorklets → `Project.new` → `startAudioWorklet` → `engine.isReady`) and returns a typed `Studio` facade: `play/stop/setPosition/setBpm`, `onPlayingChange/onPositionChange/onBpmChange/onCpuLoadChange`, `loadDemo()`, plus `project`/`engine`/`audioContext` escape hatches. This is the **only** file that imports the SDK. **Typecheck/build verified, but runtime is unverified** — it needs a real browser (crossOriginIsolated + user gesture), which can't run from this WSL shell.
...
- **UI shell (throwaway)** — transport bar, track list, Canvas timeline, mixer, backed by a Solid store at `src/state/project.ts`. This store and its playback/meters are **UI-driven placeholders not yet connected to the engine**; the UI is being redesigned, so don't over-invest here.

**Next step:** connect the UI to the `Studio` facade and migrate the source of truth from the Solid store onto the engine's project model (BoxGraph). Verify audio in a browser via the smoke hook first.
