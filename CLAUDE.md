...
**Next step:** connect the (redesigned) UI to `studio.mixer` — render `mixer.subscribe()` snapshots and drive `createInstrumentTrack`/`setVolume`/etc. — retiring the throwaway Solid store at `src/state/project.ts`.
