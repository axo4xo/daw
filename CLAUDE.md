...
- **UI (`src/ui/`)** — the FL/Ableton-style **"Looper"** interface, ported from the user's HTML mockup to Solid + CSS (`src/index.css` design system). `Looper.tsx` holds the title bar, transport, browser, and the Arrange / Mix / (placeholder) Session views; `decor.ts` is the **static mock data** (tracks, clips, devices, waveform, browser) for the parts the engine doesn't expose yet. Fonts are system stacks — Google Fonts are blocked by COEP, so self-host if exact faces are needed.
  - **Engine-wired (live):** the transport bar (play/stop, editable BPM, bars·beats + time, CPU), the Arrange playhead, and the **whole Mix view** — channel strips + master from `studio.mixer`, with interactive pointer-drag `Fader`/`PanKnob` (→ `setVolume`/`setPan`) and M/S buttons (→ `setMute`/`setSolo`). Boot is on mount; 6 tracks are auto-seeded (one with a demo loop) so it's populated and audible.
  - **Still mock (decor):** the Arrange track lanes/clips, the browser, the bottom Sample/Effects device racks. The logo and Session tab were removed per the user.

**Next step:** engine-back the Arrange view — real track headers (mute/solo) and real clips from note/audio regions — and add **live meters on Canvas** (`Fader.meter` is currently a placeholder mirroring volume; the engine exposes per-unit level for a real meter).
